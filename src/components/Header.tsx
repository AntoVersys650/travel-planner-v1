...
const LanguageSwitcher = ({ currentLanguage, onLanguageChange }: LanguageSwitcherProps) => {
  ...
  // Oggetto che mappa i nomi tradotti per ciascuna lingua in base alla lingua corrente
  ...
  // Label dinamica per il selettore (tradotta in base alla lingua corrente)
  ...
  // Stili uniformi per il bottone e il dropdown (uguali a quelli degli altri menu)
  const buttonStyle = {
  ...
  return (
    <div className="language-switcher" ref={selectRef} style={{ position: 'relative', display: 'inline-block' }}>
      <button onClick={toggleOpen} style={buttonStyle}>
        <span style={{ marginRight: '5px' }}>{languageLabels[currentLanguage]}</span>
        <img
